...
export const clienteRoutes = (pool: Pool) => {
  ...
  router.post('/api/cliente/carga-masiva', async (req: Request, res: Response) => {
    ...
    if (!req.files || !req.files.file) {
      ...
      console.log('Datos (primeros 200 caracteres):', file.data.toString('utf8').substring(0, 200));
      
      // Guardar archivo temporal para inspección
      const fs = require('fs');
      const tempPath = `/tmp/debug_${Date.now()}.csv`;
